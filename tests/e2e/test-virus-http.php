...
/**
 * End-to-end HTTP test for the virus-scan wiring in index.php. The FileManager seam
 * is covered by tests/integration/test-virus-scan.php with injected scanners; what
 * can only be checked through the real router is the WIRING decision itself:
 *
 *   - `allow_virus_scan` off → uploads behave exactly as before (no cost, no gate);
 *   - `allow_virus_scan` on with the proprietary module absent (this MIT checkout)
 ...
 *
 * Usage: php tests/e2e/test-virus-http.php   (requires the curl extension)
 */
...
function upload(string $base, string $tok, string $disk, string $path, string $bytes): array {
    $tmp = tempnam(sys_get_temp_dir(), 'ffv'); file_put_contents($tmp, $bytes);
...
echo "\n{$cyan}══ Virus-scan wiring over HTTP (e2e) ══{$reset}\n\n";

$prefix = 'virus_e2e';
try {
    $tokOff = fluxfiles_token('virus-off', ['read', 'write'], ['local'], $prefix, 50, null, 3600);
    ...
    $tokOn = FluxFiles\JwtCompat::encode(array_merge(
        ...
        ['sub' => 'virus-on', 'allow_virus_scan' => true]
    ), $SECRET);
    ...
    test('claim ON + module absent → 501 module_not_installed (fails closed)', function () use ($BASE, $tokOn) {
        ...
        assertEqual('virus', $j['error_params']['module'] ?? null);
    });
    ...
    test('chunk upload is refused while scanning is on (no unscannable side door)', function () use ($BASE, $tokOn) {
        foreach (['init', 'presign', 'complete', 'abort'] as $step) {
            ...
            assertEqual('virus_unscannable', $j['error_code'] ?? null, "chunk/{$step} code");
        }
    });

    test('chunk upload still works normally when scanning is off', function () use ($BASE, $tokOff) {
        // local disk has no multipart backend, so this must NOT be the virus refusal —
        ...
        assertTrue(($j['error_code'] ?? '') !== 'virus_unscannable', 'must not be refused for a token without the claim');
    });
