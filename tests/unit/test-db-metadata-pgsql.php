...
/**
 * Behavioral test suite for FluxFiles\Db\DbMetadataHandler over a real
 * PostgreSQL database — self-contained duplicate of
 * test-db-metadata-sqlite.php's body against the `pgsql` dialect. Skips
 * (exit 0) when FXTEST_DB_PGSQL_DSN is not set, so it's safe to pick up in
 * the blanket unit-test loop on every job, including ones with no Postgres
 * service.
 *
 * Usage:
 *   FXTEST_DB_PGSQL_DSN="pgsql:host=127.0.0.1;dbname=fluxfiles_test" \
 *   FXTEST_DB_PGSQL_USER=postgres FXTEST_DB_PGSQL_PASSWORD=postgres \
 *   php tests/unit/test-db-metadata-pgsql.php
 */
...
$dsn = getenv('FXTEST_DB_PGSQL_DSN');
if (!$dsn) {
    echo "\n{$cyan}══ DbMetadataHandler (PostgreSQL) Test Suite ══{$reset}\n";
    echo "  {$yellow}SKIP{$reset} — FXTEST_DB_PGSQL_DSN not provided\n\n";
    exit(0);
}
...
$user = getenv('FXTEST_DB_PGSQL_USER') ?: '';
$pass = getenv('FXTEST_DB_PGSQL_PASSWORD') ?: '';
$storageRoot = sys_get_temp_dir() . '/ff_test_db_metadata_pgsql_' . getmypid();
...
echo "\n{$cyan}╔══════════════════════════════════════════════════╗{$reset}\n";
echo "{$cyan}║      DbMetadataHandler (PostgreSQL) Test Suite    ║{$reset}\n";
echo "{$cyan}╚══════════════════════════════════════════════════╝{$reset}\n\n";
...
foreach (['file_metadata', 'directories', 'trash', 'audit_log', 'rate_limits', 'legal_holds'] as $table) {
    $conn->pdo()->exec("DELETE FROM {$table}");
}
exec('rm -rf ' . escapeshellarg($storageRoot));
