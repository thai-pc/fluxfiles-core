...
test('optimizeFiles sends format:avif when the optimize_format claim is avif', async ({ page }) => {
  await openManager(page, mintTokenWithClaims({ allow_optimize: true, optimize_format: 'avif' }));
  let posted: any = null;
  await page.route('**/api/fm/optimize', async (route) => {
    posted = route.request().postDataJSON();
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { path: 'a.avif', saved_bytes: 1, saved_pct: 50 } }) });
  });
  ...
  await page.evaluate(() => (window as any).Alpine.$data(document.querySelector('.ff-app')).optimizeFiles(['photos/a.jpg']));
  expect(posted).toMatchObject({ disk: 'local', path: 'photos/a.jpg', format: 'avif' });
});
